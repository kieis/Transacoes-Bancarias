...
void salvarConta(ContaCorrente* contaAtual)
{
	...
	if (arquivo)
	{
		...
		for (int i = 0; i < linhasL.size(); i++)
		{
			...
			if (atoi(colunasL[0].c_str()) == contaAtual->conta && atoi(colunasL[1].c_str()) == contaAtual->digito && atoi(colunasL[3].c_str()) == contaAtual->agencia)
				iProcura = i;
		}

		vector<string> colunas;
		colunas.push_back(tostr(contaAtual->conta));
		colunas.push_back(tostr(contaAtual->digito));
		colunas.push_back(contaAtual->cpf);
		colunas.push_back(tostr(contaAtual->agencia));
		colunas.push_back(contaAtual->nome);
		colunas.push_back(tostr(contaAtual->saldo));
	...
	void criar(ContaCorrente* contasLista[], Agencia* agenciaLista[])
	{
		...
		if (!vP(procurar)) {
			...
			if (vP(contasLista[FIM]) && Anterior != contasLista[FIM])
			{
				...
				{
					vector<string> colunas;
					colunas.push_back(tostr(leitura.conta));
					colunas.push_back(tostr(leitura.digito));
					colunas.push_back(leitura.cpf);
					colunas.push_back(tostr(leitura.agencia));
					colunas.push_back(leitura.nome);
					colunas.push_back(tostr(leitura.saldo));
