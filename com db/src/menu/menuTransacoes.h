...
TransacoesBancarias* criarTransacao(TransacoesBancarias* transacaoAtual, Caixa* caixa, SenhaAtendimento senhaInfo, const char* tipoTransacao, ContaCorrente* contaOrigem, ContaCorrente* contaDestino, float valor)
{
	...
	if (!vP(contaDestino))
	{
		novaTransacao->destino[0/*conta*/] = 0;
		novaTransacao->destino[1/*digito*/] = 0;
		novaTransacao->destino[2/*agencia*/] = 0;
		novaTransacao->destinoSaldo = 0.0f;
	}

	arquivoIO* arquivoConta = new arquivoIO(obterDiretorio(), FILE_CONTA, FILE_ALTERACAO),
		     * arquivoCaixa = new arquivoIO(obterDiretorio(), FILE_CAIXA, FILE_ALTERACAO),
		     * arquivoTransacoes = new arquivoIO(obterDiretorio(), FILE_TRANSACOES, FILE_ALTERACAO);

	if (!vP(arquivoConta)) {
		free(novaTransacao);
		printf("\n-> Falha no acesso aos dados da conta em disco.");
		return transacaoAtual;
	}

	if (!vP(arquivoTransacoes)) {
		free(novaTransacao);
		printf("\n-> Falha no acesso aos dados da transação em disco.");
		return transacaoAtual;
	}
	...
	if (strstr(tipoTransacao, transacoes[0])) //SQ
	{
		if (!vP(arquivoCaixa)) {
			free(novaTransacao);
			printf("\n-> Falha no acesso aos dados do caixa em disco.");
			return transacaoAtual;
		}
	...
	colunas.push_back(tostr(novaTransacao->dataHora->dia));
	colunas.push_back(tostr(novaTransacao->dataHora->mes));
	colunas.push_back(tostr(novaTransacao->dataHora->ano));
	colunas.push_back(tostr(novaTransacao->dataHora->hora));
	colunas.push_back(tostr(novaTransacao->dataHora->min));
	colunas.push_back(tostr(novaTransacao->dataHora->seg));
	colunas.push_back(tostr(novaTransacao->caixa->numero));
	colunas.push_back(tostr(novaTransacao->senhaInfo.dtH->dia));
	colunas.push_back(tostr(novaTransacao->senhaInfo.dtH->mes));
	colunas.push_back(tostr(novaTransacao->senhaInfo.dtH->ano));
	colunas.push_back(tostr(novaTransacao->senhaInfo.dtH->min));
	colunas.push_back(tostr(novaTransacao->senhaInfo.dtH->hora));
	colunas.push_back(tostr(novaTransacao->senhaInfo.dtH->seg));
	colunas.push_back(tostr(novaTransacao->senhaInfo.numero));
	colunas.push_back(tostr(novaTransacao->senhaInfo.status));
	colunas.push_back(novaTransacao->tipo);
	colunas.push_back(tostr(novaTransacao->valor));
	colunas.push_back(tostr(novaTransacao->origemSaldo));
	colunas.push_back(tostr(novaTransacao->destinoSaldo));
	colunas.push_back(tostr(novaTransacao->origem[0]));
	colunas.push_back(tostr(novaTransacao->origem[1]));
	colunas.push_back(tostr(novaTransacao->origem[2]));
	colunas.push_back(tostr(novaTransacao->destino[0]));
	colunas.push_back(tostr(novaTransacao->destino[1]));
	colunas.push_back(tostr(novaTransacao->destino[2]));

	arquivoTransacoes->escreverColunas(colunas, ';');

	free(arquivoConta);
	free(arquivoCaixa);
	free(arquivoTransacoes);
...
TransacoesBancarias* carregarTransacao(TransacoesBancarias* transacaoAtual, DataHora* dthTransacao, Caixa* caixa, SenhaAtendimento senhaInfo, const char* tipoTransacao, ContaCorrente* contaOrigem, ContaCorrente* contaDestino, float saldoOrigem, float saldoDestino, float valor)
{
	if (!vP(transacaoAtual))
		return NULL;
	...
	novaTransacao->dataHora = dthTransacao;
	novaTransacao->caixa = caixa;
	novaTransacao->senhaInfo = senhaInfo;
	novaTransacao->valor = valor;
	sprintf(novaTransacao->tipo, "%s", tipoTransacao);

	novaTransacao->destino[0/*conta*/] = (vP(contaDestino) ? contaDestino->conta : 0);
	novaTransacao->destino[1/*digito*/] = (vP(contaDestino) ? contaDestino->digito : 0);
	novaTransacao->destino[2/*agencia*/] = (vP(contaDestino) ? contaDestino->agencia : 0);
	novaTransacao->destinoSaldo = (vP(contaDestino) ? saldoDestino : 0);

	novaTransacao->origem[0/*conta*/] = contaOrigem->conta;
	novaTransacao->origem[1/*digito*/] = contaOrigem->digito;
	novaTransacao->origem[2/*agencia*/] = contaOrigem->agencia;
	novaTransacao->origemSaldo = saldoOrigem;
...
{
	void iniciar(TransacoesBancarias* transacoesLista[], Caixa* caixasLista[], ContaCorrente* contasLista[]);
