...
Caixa* carregarCaixa(Caixa* caixaAtual, int numero, int matricula, int senhasAtendidas, bool caixaAtivo, const char* operador, float Saldo[], DataHora* dtAbertura, DataHora* dtFechamento)
{
	if (!vP(caixaAtual))
		return NULL;
	...
	novoCaixa->numero = numero;
	novoCaixa->matricula = matricula;
	novoCaixa->senhasAtendidas = senhasAtendidas;
	sprintf(novoCaixa->operador, "%s", operador);
	novoCaixa->caixaAtivo = caixaAtivo;

	novoCaixa->saldo[SALDOCAIXA::INICIAL] = Saldo[SALDOCAIXA::INICIAL];
	novoCaixa->saldo[SALDOCAIXA::ATUAL] = Saldo[SALDOCAIXA::ATUAL];
	novoCaixa->saldo[SALDOCAIXA::REPOSICAO] = Saldo[SALDOCAIXA::REPOSICAO];

	novoCaixa->dtAbertura = dtAbertura;
	novoCaixa->dtFechamento = dtFechamento;
...
void salvarCaixa(Caixa* caixaAtual)
{
	...
	if (arquivo)
	{
		...
		for (int i = 0; i < linhasL.size(); i++)
		{
			...
			if (atoi(colunasL[0].c_str()) == caixaAtual->numero)
				iProcura = i;
		}

		vector<string> colunas;
		colunas.push_back(tostr(caixaAtual->numero));
		colunas.push_back(tostr(caixaAtual->matricula));
		colunas.push_back(tostr(caixaAtual->senhasAtendidas));
		colunas.push_back(caixaAtual->operador);
		colunas.push_back(tostr(caixaAtual->saldo[SALDOCAIXA::INICIAL]));
		colunas.push_back(tostr(caixaAtual->saldo[SALDOCAIXA::ATUAL]));
		colunas.push_back(tostr(caixaAtual->saldo[SALDOCAIXA::REPOSICAO]));
		colunas.push_back(tostr(caixaAtual->caixaAtivo));

		colunas.push_back(tostr(vP(caixaAtual->dtAbertura) ? caixaAtual->dtAbertura->dia : 0));
		colunas.push_back(tostr(vP(caixaAtual->dtAbertura) ? caixaAtual->dtAbertura->mes : 0));
		colunas.push_back(tostr(vP(caixaAtual->dtAbertura) ? caixaAtual->dtAbertura->ano : 0));
		colunas.push_back(tostr(vP(caixaAtual->dtAbertura) ? caixaAtual->dtAbertura->hora : 0));
		colunas.push_back(tostr(vP(caixaAtual->dtAbertura) ? caixaAtual->dtAbertura->min : 0));
		colunas.push_back(tostr(vP(caixaAtual->dtAbertura) ? caixaAtual->dtAbertura->seg : 0));

		colunas.push_back(tostr(vP(caixaAtual->dtFechamento) ? caixaAtual->dtFechamento->dia : 0));
		colunas.push_back(tostr(vP(caixaAtual->dtFechamento) ? caixaAtual->dtFechamento->mes : 0));
		colunas.push_back(tostr(vP(caixaAtual->dtFechamento) ? caixaAtual->dtFechamento->ano : 0));
		colunas.push_back(tostr(vP(caixaAtual->dtFechamento) ? caixaAtual->dtFechamento->hora : 0));
		colunas.push_back(tostr(vP(caixaAtual->dtFechamento) ? caixaAtual->dtFechamento->min : 0));
		colunas.push_back(tostr(vP(caixaAtual->dtFechamento) ? caixaAtual->dtFechamento->seg : 0));

		if (iProcura != -1)
			arquivo->reescreverLinha(colunas, iProcura, ';');
		else
			arquivo->escreverColunas(colunas, ';');
